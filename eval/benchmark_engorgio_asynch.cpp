...
int main(int argc, char** argv)
{
    ...
    options.add_options()
    ("g,gpus", "Number of GPUs", cxxopts::value<int>()->default_value("4"))
    ("v,videos", "Number of videos", cxxopts::value<int>()->default_value("32"))
    ("i,instance", "Instance name", cxxopts::value<std::string>()->default_value("g4dn.12xlarge"))
    ("r,resolution", "Resolution", cxxopts::value<int>()->default_value("720"))
    ("f,fraction", "Fraction of anchor frames", cxxopts::value<double>()->default_value("0.075"))
    ("a,framerate", "Frame rate", cxxopts::value<int>()->default_value("60"))
    ("d,duration", "Duration", cxxopts::value<int>()->default_value("600"))
    ("n,number", "Number of frames", cxxopts::value<int>()->default_value("0"))
    ...
    int num_frames = result["number"].as<int>();
    ...
    if (num_frames == 0)
        num_frames = int(framerate * duration * 0.9);
    ...
    // std::filesystem::path log_dir = std::filesystem::path(ENGORGIO_RESULT_DIR) / "evaluation" / instance_name / log_subdir;
    std::filesystem::path log_dir = std::filesystem::path(ENGORGIO_RESULT_DIR) / "evaluation" / instance_name / resolution_name / model_name / log_subdir;
